...
3. **표지·ISBN 보강 (Naver Books API)**:
   - 기존 `scripts/books/fetch-naver-books.ts` 패턴을 재사용. 서명+저자를 쿼리로 던져 첫 매치를 사용.
   - 동명 책·시리즈로 인한 오매칭을 줄이기 위해 매칭 정확도 휴리스틱: 정규화된 서명(괄호·부제 제거)이 50% 이상 일치할 때만 채택. 불일치면 표지 없음으로 fallback.
