...
export function MobileOnlyPage({ nextPath }: { nextPath: string }) {
  ...
  return (
    <main className="scanline relative grid min-h-[100svh] overflow-hidden bg-bg-primary px-4 py-4 text-text-primary">
      ...
      <section className="relative z-10 mx-auto flex w-full max-w-[23rem] flex-col justify-start pt-5">
        <div className="mb-3 inline-flex w-fit items-center gap-2 rounded-2xl border border-border bg-bg-card/72 p-1.5 pr-3.5 shadow-glass backdrop-blur-2xl">
          <span className="grid h-10 w-10 place-items-center rounded-xl border border-border bg-bg-card/80">
            <BrandLogo className="h-9 w-9 object-contain" />
          </span>
          <span className="text-sm font-black uppercase tracking-[0.03em] text-text-muted">AI 관상가 고양이</span>
        </div>

        <div className="glass-panel rounded-[1.75rem] p-5 shadow-2xl shadow-black/10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h1 className="mt-0.5 text-[1.68rem] font-bold leading-tight text-text-primary">PC에서 접속해 주세요</h1>
            </div>
          </div>

          <p className="mt-4 text-[0.96rem] font-semibold leading-6 text-text-muted">
            실시간 카메라와 가로형 결과 리포트가 필요해요. 모바일에서는 야옹이가 확대경을 제대로 못 듭니다.
          </p>

          <div className="mt-4 rounded-2xl border border-border/70 bg-bg-card/55 p-4">
            <div className="flex items-center gap-2.5">
              <Laptop className="h-5 w-5 shrink-0 text-accent-info" aria-hidden="true" />
            ...
            <div className="mt-2 flex items-center gap-2.5">
              <Smartphone className="h-5 w-5 shrink-0 text-accent-info" aria-hidden="true" />
              <p className="text-sm font-bold leading-6 text-text-muted">
          ...
          <div className="mt-4 rounded-2xl border border-border bg-bg-card/65 p-2">
            <p className="truncate rounded-xl bg-bg-raised/45 px-3 py-2.5 text-sm font-bold leading-5 text-text-primary">{pcUrl}</p>
            <button
              type="button"
              onClick={copyUrl}
