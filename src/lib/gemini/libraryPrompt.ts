...
export function buildLibraryPrompt({
  ...
  return [
    ...
    "- romantic_match는 관계 궁합 관점으로 작성한다. best_types는 1~2개만 쓰고, 첫 번째 유형을 가장 확실한 매칭으로 둔다. 왜 잘 맞는지, 함께하기 좋은 흐름, 조심할 점을 고양이 상담 톤으로 구체적으로 적는다. 사용자에게 보이는 문장에는 '연애'와 '데이트'라는 단어를 쓰지 않는다.",
